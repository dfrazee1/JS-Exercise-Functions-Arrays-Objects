 ...
 * }
*/
function makePersonObject(object1, object2, object3) {
  const obj = {
    id: object1,
    name: object2,
    email: object3
...
function getCarInfoByIndex(inventory, index) {
  const stuff = inventory[index];
  return `This is a ${stuff.car_make} ${stuff.car_model}`;
}
...
function getLastCarInfo(inventory) {
  const stuff = inventory[inventory.length - 1];
  return `This is a ${stuff.car_make} ${stuff.car_model}`;
}
...
function getGermanCars(inventory) {
  ...
  for (i=0; i<inventory.length; i++) {
    if (['Audi', 'Mercedes-Benz', 'Volkswagen', 'BMW'].includes(inventory[i].car_make)) {
      newArray.push(inventory[i]);
    }
  }
  return newArray;
